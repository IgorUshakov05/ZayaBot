...
export default async function payUsers(payments: IPaymentMethod[]): Promise<{
  success: boolean;
  message: string;
  processed: number;
  failed: number;
  ...
  try {
    if (payments.length === 0) {
      return {
        success: true,
        message: "Нет платежей для обработки",
        processed: 0,
        failed: 0,
      };
    }
    ...
    for (let i = 0; i < payments.length; i++) {
      ...
      try {
        if (payment.saved) {
          console.log(
            `Обработка автоплатежа для пользователя ${payment.user.chat_id}`
          );
          let email = payment.user.mail ? payment.user.mail : conf.ADMIN_MAIL;
          ...
          if (paymentResult.success) {
            processedCount++;
            console.log(
              `✓ Автоплатеж успешен для пользователя ${payment.user.chat_id}`
            );
          } else {
            failedCount++;
            usersWithoutSavedPay.push(payment.user.chat_id);
            console.log(
              `✗ Автоплатеж не удался для пользователя ${payment.user.chat_id}`
            );
          }
        } else {
          // Для пользователей без сохраненного платежа
          console.log(
            `Отправка уведомления пользователю ${payment.user.chat_id}`
          );
          ...
          if (messageResult.success) {
            console.log(
              `✓ Уведомление отправлено пользователю ${payment.user.chat_id}`
            );
          } else {
            console.log(
              `✗ Не удалось отправить уведомление пользователю ${payment.user.chat_id}`
            );
          }
        ...
        console.error(
          `Ошибка обработки пользователя ${payment.user.chat_id}:`,
          error
        );
        failedCount++;
        usersWithoutSavedPay.push(payment.user.chat_id);
    ...
    if (usersWithoutSavedPay.length > 0) {
      console.log(
        `Сброс тарифа для ${usersWithoutSavedPay.length} пользователей...`
      );
      const resetResult = await set_user_tariff(usersWithoutSavedPay);

      if (resetResult.success) {
        console.log(
          `✓ Тарифы сброшены для ${resetResult.updatedCount} пользователей`
        );
      } else {
